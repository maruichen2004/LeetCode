class Twitter(object):
    ...
    def getNewsFeed(self, userId):
        ...
        for user in self.friends[userId]:
            for time, tweet in self.tweets[user].items():
                if len(top10) > 0 and top10[0][0] > time and len(top10) > 10:
                    break
                heapq.heappush(top10, (time, tweet))
                if len(top10) > 10:
                    heapq.heappop(top10)
        while top10:
            res.insert(0, heapq.heappop(top10)[1])
        return res
